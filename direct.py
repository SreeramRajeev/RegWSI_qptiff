...
def register_qptiff_direct(he_qptiff_path: Path, if_qptiff_path: Path, 
    ...
    try:
        ...
        if disp_field_path.exists():
            ...
            # Also save grayscale result for comparison
            warped_gray = reg_dir / "warped_source.tiff"
            if warped_gray.exists():
                results['registered_gray'] = warped_gray
        
        else:
            print("   ❌ Displacement field not found!")
            results['success'] = False
            results['error'] = "Displacement field not generated"
